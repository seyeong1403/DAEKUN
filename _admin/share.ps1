...
$srvArgs = @('-NoProfile', '-File', $serverPs, '-NoBrowser', '-Port', $Port, '-Password', $Password, '-User', $User)
...
$logFile = Join-Path $env:TEMP ('daekun-tunnel-' + (Get-Date).ToString('HHmmss') + '.log')
...
# ---------- 3. 안내 ----------
$mode = if ($Editable) { '수정 · 배포까지 가능' } else { '구경만 가능 (저장 · 배포 잠김)' }

Write-Host ''
Write-Host '  ================================================================'
Write-Host '   대건엠에스 관리자 - 외부 공유 주소' -ForegroundColor Cyan
Write-Host '  ================================================================'
Write-Host ''
