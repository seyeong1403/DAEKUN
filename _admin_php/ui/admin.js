...
/* -------------------------------------------------- 잘못 연 경우 안내
   index.html 을 직접 열면(file://) 서버와 통신할 수 없다.
   반드시 웹 주소(https://도메인/_admin/)로 들어와야 한다. */
if (location.protocol === 'file:') {
	document.body.innerHTML =
		...
		'<p style="color:#5A6172;font-size:15px">지금 파일을 직접 열어서 들어오셨습니다. 이렇게 열면 관리자가 동작하지 않습니다.</p>' +
		'<p style="color:#5A6172;font-size:15px;margin-top:14px"><b style="color:#1A1E28">홈페이지 주소 뒤에 ' +
		'<b style="color:#1A1E28">/_admin/</b> 을 붙여서 들어와 주세요.</p>' +
		'<p style="margin-top:22px;padding:14px 16px;background:#F5F6F8;border-radius:8px;font-size:14px;color:#5A6172">' +
		'예를 들어 이렇게 입력합니다<br>' +
		'<b style="color:#3E6EAF;font-size:15px">https://daekunms.co.kr/_admin/</b></p></div>';
	return;
}
...
/* 다른 사이트에서 몰래 요청을 보내는 것을 막는 값. index.php 가 심어 준다. */
const DK_CSRF = (document.querySelector('meta[name="dk-csrf"]') || {}).content || '';

async function api(route, body) {
	/* route 는 'state' 또는 'file?path=...' 형태로 들어온다.
	   api.php?r=state / api.php?r=file&path=... 로 바꿔 준다. */
	const cut = route.indexOf('?');
	const name = cut < 0 ? route : route.slice(0, cut);
	const qs = cut < 0 ? '' : route.slice(cut + 1);
	const url = 'api.php?r=' + encodeURIComponent(name) + (qs ? '&' + qs : '');

	const opt = body
		? {
			method: 'POST',
			headers: { 'Content-Type': 'application/json', 'X-CSRF': DK_CSRF },
...
views.dash = async function () {
	try {
		...
		const wired = !!STATE.inqWired;
		const checks = [
			['검색엔진 색인 차단(noindex)을 해제했습니다. 검색에 노출됩니다.', true],
			['약관 · 개인정보처리방침의 [ 대괄호 ] 자리를 채워야 합니다.', false],
			[wired
				? '문의 폼이 접수 서버에 연결되어 있습니다. 방문자 문의가 이 화면에 쌓입니다.'
				: '문의 폼이 아직 연결되지 않았습니다. contact/inquiry.html 의 data-endpoint 를 /_admin/inquiry.php 로 바꿔 주세요.',
				wired],
...
async function renderInqEndpoint() {
	...
	$('#inqEndpoint').innerHTML = `
		...
		<p class="panel-desc" style="color:var(--txt-3)">문의 폼은 <b>inquiry.php</b> 로 접수됩니다. 방문자가 보낸 문의가 이 화면에 바로 쌓입니다.</p>
